        ...
        foreach ($dienstverband in $xml.GetElementsByTagName("dienstverband")) {
            ...
            if ([string]::IsNullOrEmpty($contract.dv_aanvullendeRubriek_E2078 ) -eq $true) {
                foreach ($name in $locationsMemberNames) {
                    $contract | Add-Member -MemberType NoteProperty -Name ("dv_" + "aanvullendeRubriek_E2078" + "_" + $name) -Value "" -Force
                }
            }
            else
            {
                ...
                foreach ($name in $locationsMemberNames) {
                     $contract | Add-Member -MemberType NoteProperty -Name ("dv_" + "aanvullendeRubriek_E2078" + "_" + $name) -Value $contractLocation."$name" -Force
                }
            }
...
$locationCsv = "C:\HelloID-Supportive\Location\locations.csv"
...
$locations = Import-Csv -Path $locationCsv
